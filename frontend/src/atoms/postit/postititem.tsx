...
const Postit: React.FC<PostitProps> = ({accessToken,postItem, color, onClick, children, style,firstData }) => {
  
  const [isLike, setIsLike] = useState(postItem && postItem.isLike || false)
  const [isReport, setIsReport] = useState(postItem && postItem.isReport || false)
  ...
  const ReportHandle = async () => {
    try {
      ...
      await axios.post(`https://mindtrip.site/api/postits/v1/report/${postItem.id}`, {}, {
        headers : {
          Authorization: accessToken
        }
      }).then(() => {
      ...
      await axios.delete(`https://mindtrip.site/api/postits/v1/report/${postItem.id}`, {
        headers : {
          Authorization: accessToken
        }
      }).then(() => {
