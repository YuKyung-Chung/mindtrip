...
function House() {
  ...
  // 처음에 질문 불러오기
  let surveyList = useSelector((state:RootState) => state.htpSurveys.house)
...
function HouseDraw({ goSurvey }: propsType1) {
  ...
  return (
    <div className="flex h-screen w-screen justify-center items-center flex-col relactive">
      <p className="text-center mb-7 font-bold text-3xl">집을 그려주세요.</p>
        ...
        survey.choices.map((choice, idx) => {
          return (
            <Button
              key={idx}
              variant="bordered"
              className='w-[90vw] lg:w-3/5 m-3 h-[10vh] px-3 text-md shadow'
